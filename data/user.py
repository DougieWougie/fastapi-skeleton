...
def create(user: User, table: str = "user") -> User:
    """Add <user> to user or dropped_user table"""
    if table not in ("user", "dropped_user"):
...
def delete(name: str) -> None:
    """Drop user with <name> from user table, add to dropped_user table"""
    user = get_one(name)
    ...
    if curs.rowcount != 1:
        raise Missing(msg=f"User {name} not found")
    create(user, table="dropped_user")
